...
# ---------------------------------------------------------------------------
# 流式 tool-use — 用于 agent loop 最后一轮
# ---------------------------------------------------------------------------


class StreamWithToolsResult:
    """容器：持有流式 tool-use 最后一轮的统计信息."""
...
async def astream_final_response(
    *,
    ...
    base_url: str | None = None,
    holder: StreamWithToolsResult | None = None,
) -> AsyncIterator[str]:
    """流式调用 LLM（带 tools schema）— 用于 agent loop 最后一轮.

    假定当前轮不会再有 tool_calls（由调用者保证），
    以 text_stream 模式逐 token yield。

    完成后通过 holder.result 获取 ToolExecutionResult。
    """
    _holder = holder or StreamWithToolsResult()
    provider = detect_provider(model)
    resolved_key = resolve_api_key(provider, api_key)

    if provider == Provider.ANTHROPIC and not base_url:
        anthropic = _get_anthropic()
        if anthropic is None:
        ...
        _holder.result = ToolExecutionResult(
            content="".join(collected),
        ...
        # 非 Anthropic 原生：fallback 到非流式，一次性 yield 全部
