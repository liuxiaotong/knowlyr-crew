...
allowed-tools: Read Bash(git:*) Grep Glob Bash
...
1. **阅读代码**：仔细阅读 $0 及其相关文件
2. **分析依赖**：用 grep 搜索 `from $0 import` 和 `import $0` 找出所有依赖者
3. **识别问题**：按重构目标 $1 找出需要改进的点
...
- **依赖数**：import 数量变化
- **复杂度**：如项目有 radon，运行 `radon cc $0 -s` 测量圈复杂度
