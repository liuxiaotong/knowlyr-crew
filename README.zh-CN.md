...
[摘要](#摘要-abstract) · [问题陈述](#问题陈述-problem-statement) · [形式化框架](#形式化框架-formal-framework) · [架构](#架构-architecture) · [核心创新](#核心创新-key-innovations) · [快速开始](#快速开始-quick-start) · [员工规范](#员工规范-employee-specification) · [生产服务器](#生产服务器-production-server) · [CLI 参考](#cli-参考-cli-reference) · [生态系统](#生态系统-ecosystem) · [参考文献](#参考文献-references)
...
现有 AI Agent 框架将智能体视为无状态函数调用——每次交互从零开始，认知成果无法积累，同一错误跨场景反复出现。knowlyr-crew 提出三层架构解决这一根本矛盾：

1. **定义层** (Specification)——声明式员工规范（Soul 灵魂配置 + YAML 元数据 + Markdown 指令），将"员工是谁"与运行时完全解耦
2. **协作层** (Deliberation)——结构化辩证协商打破群体偏见（Janis, 1972）和共享信息偏差（Stasser & Titus, 1985），9 种交互模式 + 流水线编排 + 异步委派
3. **进化层** (Evolution)——指数衰减持久记忆（Ebbinghaus 遗忘曲线）+ 评估反馈闭环（与 RLHF 机制同构，Christiano et al., 2017）+ Skills 自动触发，让员工从经验中持续进化

三层统一暴露为 MCP 协议，实现「**定义 → 协商 → 决策 → 评估 → 记忆更新 → 自动触发**」的自校正闭环——人类反馈直接反馈为 Agent 的持久化记忆，塑造后续推理行为。

> **knowlyr-crew** 不是又一个编排框架，而是 AI 数字员工的**能力定义层**、**协作执行层**和**经验积累层**。系统暴露 40 个 MCP 工具、支持 3 种传输协议，路由至 7 个 LLM 供应商，通过飞书、企业微信、Web 多端触达。
...
| **经验断裂**<br/>Experience Fragmentation | 员工切换场景后失去所有积累的上下文知识，同一错误跨场景反复出现 | 无上下文感知的自动触发机制，记忆与场景脱节 | Skills 自动触发（语义/关键词/常驻三种模式）+ Soul 持久身份 + 跨场景记忆自动注入 |

> Crew 不是又一个编排框架。它是 AI 数字员工的**能力定义层**、**协作执行层**和**经验积累层**——"谁是谁、怎么协商、学到了什么"，而将身份管理和运行时交互交给 [knowlyr-id](https://github.com/liuxiaotong/knowlyr-id)。
...
| 层 | 模块 | 职责 |
|:---|:---|:---|
| **Specification** | Parser · Discovery · Models · Soul Store | 声明式员工定义解析，YAML/Markdown 双格式，Soul 灵魂配置（自动版本递增 + 历史追踪），优先级发现 |
| **Protocol** | MCP Server · Skill Converter · MCP Gateway | 40 Tools + Prompts + Resources，stdio/SSE/HTTP 三协议，外部 MCP 工具动态注入 |
| **Skills** | Trigger Engine · Action Executor | 语义/关键词/常驻三种触发模式，自动加载相关记忆注入 prompt，触发率统计与历史 |
...
| **Memory** | Memory Store · Semantic Index · PostgreSQL | 远程持久化，语义搜索，指数衰减，重要性排序，草稿/归档/共享/反馈，跨员工 Pattern 共享，多后端 Embedding 降级 |
| **Evaluation** | Evaluation Engine | 决策追踪，回溯评估，自动纠正记忆，过期决策提醒 |
| **Execution** | Providers · Output Sanitizer · Cost Tracker | 7 Provider 统一调用，重试/降级/逐任务成本计量，双层输出清洗（源头 + 出口） |
| **Integration** | ID Client · Webhook · Cron · Feishu · WeCom | 身份联邦（Circuit Breaker），飞书/企微/GitHub 多渠道事件路由，定时任务（巡检/复盘/KPI/知识周刊），触发型自动委派 |
| **Observability** | Trajectory · Metrics · Audit | 零侵入轨迹录制 (contextvars)，权限矩阵查询，工具调用审计日志，CI 部署后自动审计，审计失败飞书告警 |
| **Wiki** | Wiki Client · Attachment Store | 知识库空间管理，文档 CRUD，附件上传/读取/删除，AI 友好视图 |
...
| 层 | 创新点 | 核心问题 |
|:---|:---|:---|
| **定义层** | 1. 声明式员工规范 · 2. Soul 灵魂系统 · 3. 组织治理与自适应权限 | 员工是谁 |
| **协作层** | 4. 结构化辩证协商 · 5. 流水线编排 | 如何协同工作 |
| **进化层** | 6. 指数衰减持久记忆 · 7. 评估反馈闭环 · 8. Skills 自动触发 | 如何持续进化 |
| **基础设施** | 9. 成本感知编排 · 10. 输出清洗 · 11. 零侵入轨迹录制 | 跨层支撑 |

---

### 定义层 — "员工是谁"

### 1. 声明式员工规范 (Declarative Employee Specification)
...
### 2. Soul 灵魂系统 (Soul — Persistent Identity)
...
### 3. 组织治理与自适应权限 (Organization Governance & Adaptive Authority)
...
### 4. 结构化辩证协商 (Structured Dialectical Deliberation)
...
### 5. 流水线编排 (Pipeline Orchestration)
...
### 6. 指数衰减持久记忆 (Persistent Memory with Exponential Decay)
...
**高级记忆管理**：支持记忆草稿（draft → approve/reject）、归档与恢复（archive → restore）、共享池（跨员工可见的共享记忆）、使用反馈（质量评分与热度追踪）。
...
### 7. 评估反馈闭环 (Evaluation Feedback Loop)
...
三种决策类别：`estimate`（估算）/ `recommendation`（建议）/ `commitment`（承诺）。评估结论自动作为 `correction` 写入该员工持久记忆，后续推理时自动注入——Agent 从自身决策误差中更新认知。`list_overdue_decisions` 工具自动提醒过期未评估的决策，防止闭环断裂。
...
### 8. Skills 自动触发 (Skills — Context-Aware Auto-Trigger)
...
### 9. 成本感知编排 (Cost-Aware Orchestration)
...
### 10. 输出清洗——纵深防御 (Output Sanitization — Defense in Depth)
...
### 11. 零侵入轨迹录制 (Zero-Intrusion Trajectory Recording)
...
### 飞书 · 企业微信 — 多渠道触达 (Feishu · WeCom — Multi-Channel Reach)

AI 员工不仅在 IDE 中工作——通过飞书和企业微信，员工可以直接在即时通讯中响应团队需求：

| 渠道 | 触发方式 | 说明 |
|:---|:---|:---|
| **飞书** | @员工名 发消息 | 自动路由到对应员工，支持 Skills 自动触发、记忆注入 |
| **企业微信** | @员工名 发消息 | 多应用支持，员工离职自动清理绑定 |
| **Web / API** | HTTP POST | 标准 REST API，支持 SSE 流式输出 |
| **Claude Code** | `/pull 员工名` | MCP 协议调用，本地 IDE 内交互 |

所有渠道统一经过 Skills 触发检查 + 输出清洗 + 审计日志，确保行为一致性。
...
<div align="center">
<sub><a href="https://github.com/liuxiaotong">knowlyr</a> — AI 数字员工的结构化辩证协商引擎</sub>
</div>
